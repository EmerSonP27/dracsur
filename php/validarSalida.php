...
$query_registro = $cnx->query("select id_registro,id_trabajador,tipo_registro,fecha_hora from registro where date(fecha_hora)=date(now()) and id_trabajador='$id' and tipo_registro='S'");
...
if ($query_registro->rowCount()>0){ /* Ya existe, juntar las dos variables en un array */
    ...
    $query_insert = $cnx->query("insert into registro(id_trabajador,tipo_registro,fecha_hora) values('$id','S',now())");
    
    $query_registro2 = $cnx->query("select id_registro,id_trabajador,tipo_registro,fecha_hora from registro where date(fecha_hora)=date(now()) and id_trabajador='$id' and tipo_registro='S'");
